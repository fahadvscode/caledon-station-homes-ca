...
export function PageHeader({ h1, lede }: { h1: string; lede?: string }) {
  return (
    <header className="bg-brand-deep text-surface pt-10 pb-14 md:pt-14 md:pb-20">
      <Container>
...
export function PageFootForm({ formId }: { formId: string }) {
  return (
    <Section alt labelledBy={`${formId}-heading`}>
      <SectionHeading id={`${formId}-heading`}>Investor Registration</SectionHeading>
      <p className="mt-4 max-w-[62ch] text-text-muted">
        Get notified when Caledon Station Homes confirms pricing, deposit structure, and assignment
        policy. Five fields only. No purchase obligation.
      </p>
